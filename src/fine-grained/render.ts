  ...
  while (oldMarker < target.childNodes.length && newMarker < children.length) {
    ...
    if (typeof newChild !== 'object') {
      if (!(oldChild instanceof Text))
        ...
        if (isSignal(newChild))
          createEffect(
            () => (oldChild.textContent = (<Signal<string>>newChild).value)
          );
        else if (typeof newChild === 'function')
          createEffect(() => (oldChild.textContent = newChild()));
        else oldChild.textContent = newChild;
